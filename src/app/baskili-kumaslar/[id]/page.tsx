...
export default function PrintedProductDetailPage() {
    const { id } = useParams()
    const { t } = useTranslate()
    const { addItem } = useCart()

    const product = PRINTED_PRODUCTS.find(p => p.id === id)
        ...
        return (
            <div className="min-h-screen flex items-center justify-center pt-24">
                <div className="text-center space-y-4">
                    ...
                    <Link href="/baskili-kumaslar" className="text-amber-600 hover:underline">
                        Baskılı kumaşlara geri dön
                    </Link>
                </div>
            </div>
        )
    ...
    const handleWhatsAppInquiry = () => {
        const message = encodeURIComponent(
            `Merhabalar, ${product.title} ürününün fiyatını öğrenmek istiyorum. ${window.location.href}`
        )
        ...
        <main className="min-h-screen bg-slate-50 pt-24 pb-20">
            <div className="container-custom">
                ...
                <nav className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs sm:text-sm text-slate-500 mb-8">
                    ...
                    <span className="text-slate-400">/</span>
                    <Link href="/baskili-kumaslar" className="hover:text-amber-600 transition-colors">Baskılı Kumaşlar</Link>
                    <span className="text-slate-400">/</span>
                    <span className="text-slate-900 font-medium break-words">{product.title}</span>
                </nav>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
                    {/* Left: Image + Usage Areas + Highlights */}
                            ...
                            <ul className="space-y-3">
                                {product.highlights.map((highlight, i) => (
                                    <li key={i} className="flex items-start space-x-3 group">
                                        ...
                                        <span className="text-slate-700 group-hover:text-slate-900 transition-colors">{highlight}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
