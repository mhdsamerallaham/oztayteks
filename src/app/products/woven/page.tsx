...
export default function WovenFabricsPage() {
    const { t } = useTranslate()

    const sections = [
        {
            id: 'classic',
            title: t('products.woven_fabrics.categories.classic'),
            items: WOVEN_PRODUCTS.filter(p => p.category === 'classic')
        },
        {
            id: 'specialty',
            title: t('products.woven_fabrics.categories.specialty'),
            items: WOVEN_PRODUCTS.filter(p => p.category === 'specialty')
        },
        {
            id: 'luxury',
            title: t('products.woven_fabrics.categories.luxury'),
            items: WOVEN_PRODUCTS.filter(p => p.category === 'luxury')
        },
        {
            id: 'utility',
            title: t('products.woven_fabrics.categories.utility'),
            items: WOVEN_PRODUCTS.filter(p => p.category === 'utility')
        },
        {
            id: 'modern',
            title: t('products.woven_fabrics.categories.modern'),
            items: WOVEN_PRODUCTS.filter(p => p.category === 'modern')
        }
    ]
                                                ...
                                                <div className="p-8 space-y-4">
                                                    ...
                                                    <div className="pt-4 border-t border-slate-50 grid grid-cols-2 gap-y-3 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0">
                                                        ...
                                                        <div className="col-span-2 pt-2">
                                                            <Link
                                                                href={`/products/woven/${product.id}`}
                                                                className="w-full py-3 bg-slate-900 text-white text-sm font-bold rounded-xl flex items-center justify-center space-x-2 hover:bg-amber-600 transition-colors group/btn"
                                                            >
